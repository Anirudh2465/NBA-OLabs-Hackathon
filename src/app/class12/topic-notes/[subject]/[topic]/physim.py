...
app = FastAPI(title="physics Experiment Generator")
...
app.mount("/projects", StaticFiles(directory="D:\\O Labs 5\\backend\\generated_projects"), name="projects")
...
#     # Create directories
#     for dir_path in dirs.values():physics
...
def get_instruction_prompt(experiment_data):
    return f"""
    I need detailed instructions for building a physics experiment simulation website with the following details:
    ...
    1. How to structure a React frontend for this specific physics experiment
    ...
    4. How to visualize the physics concepts (collisions, projectiles, etc.)
...
def get_implementation_prompt(experiment_data, instructions):
    ...
    Generate the complete code for a physics experiment simulation website with React frontend that can run standalone in an iframe (no backend required).
...
def get_validation_prompt(project_data):
    return f"""
    Validate the following physics experiment simulation project for correctness and completeness:
    
    {json.dumps(project_data, indent=2)}
    
    Check for:
    1. Scientific accuracy of the physics simulation
...
#             </head>
#             <body>
#                 <h1>physics Simulation: ${experiment_data.experiment_name}</h1>
